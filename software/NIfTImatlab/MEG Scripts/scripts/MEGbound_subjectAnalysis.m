...
for isub = 1:length(sub)
    ...
    cd(['/Volumes/davachilab/MEGbound/Data/V2/' sub{isub}])
    ...
    for inew = 1:length(test)
        ...
        elseif test{inew,8}=='h' || test{inew,8}=='l'
    ...
    for itrial = 1:length(old_new_test)
        
        if strcmp('B',old_new_test{itrial,9})
            
            num_B_trials=num_B_trials+1;
            
        end
        
        if strcmp('NB',old_new_test{itrial,9})
    ...
    old_new_B_acc_vec = zeros(length(old_new_test),1);
    old_new_NB_acc_vec = zeros(length(old_new_test),1);
    
    for itrial = 1:length(old_new_test)
        
        if strcmp('j',old_new_test{itrial,8})&&strcmp('B',old_new_test{itrial,9})
            
            old_new_B_acc_vec(itrial)=1;
            
        end
        
        if strcmp('j',old_new_test{itrial,8})&&strcmp('NB',old_new_test{itrial,9})
            
            old_new_NB_acc_vec(itrial)=1;
    ...
    Test.OldNew.Acc.B = sum(old_new_B_acc_vec)/num_B_trials;
    Test.OldNew.Acc.NB = sum(old_new_NB_acc_vec)/num_NB_trials;
    
%    Test.OldNew.Acc.all = old_new_acc_vec;
    
    % Temporal order accuracy
    
    TO_acc_vec = zeros(length(TO_test),1);
    
    for itrial = 1:length(TO_test)
        
        if strcmp('j',TO_test{itrial,8})&&strcmp('0',TO_test{itrial,11})
            
            TO_acc_vec(itrial)=1;
            
        elseif strcmp('k',TO_test{itrial,8})&&strcmp('1',TO_test{itrial,11})
            
            TO_acc_vec(itrial)=1;
            
        elseif strcmp('h',TO_test{itrial,8})
            
            TO_acc_vec(itrial)=99;
            
        elseif strcmp('l',TO_test{itrial,8})
    ...
    figure
    bar([Test.OldNew.Acc.B Test.OldNew.Acc.NB TO_B_Acc TO_NB_Acc])
    conditions = {'Item Boundary','Item Non-Boundary','TO Boundary','TO Non-Boundary'};
    ...
    figure; 
    plot(mean(reshape(CTM_B_error,6,16)),'red')
    hold on
    plot(mean(reshape(CTM_NB_error,6,16)),'green')

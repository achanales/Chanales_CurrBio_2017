...
try
    ...
    for iTrial = 1:nTrials
        ...
        PTBDisplayPictures(itemfile(iTrial), {[PTBScreenRes.width*.5 PTBScreenRes.height*.33]}, {1}, {StimDur,'any'}, 'flag', 1, 0);
        trialOnset = GetSecs;
        
        %use this to collect responses
        PTBPlaySoundFile('blankWav100ms.wav',{'end'},8);
        SoundOnset = GetSecs;
        
        responseOnset = GetSecs;
        RT = responseOnset - trialOnset;
        
        if GetSecs - PTBLastPresentationTime < 2.5
        PTBDisplayMatrices({currcolorbox},{[PTBScreenRes.width*.5 PTBScreenRes.height*.33]},{-1},num2str(rgbcolor(iTrial,:)));
        PTBDisplayMatrices({whitebox},{[PTBScreenRes.width*.5 PTBScreenRes.height*.33]},{-1},'white')
        PTBDisplayPictures(itemfile(iTrial), {[PTBScreenRes.width*.5 PTBScreenRes.height*.33]}, {1}, {2.5 - (GetSecs - PTBLastPresentationTime)}, 'flag');
        end
...
%                 
%                 %Play a sound when paused with 'p', and then send trigger
%                 PTBPlaySoundFile('Sample.wav',{'end'},8);
%                 
%                 
%                 pause(.1)
%                 
...
%                         if strcmp(kbname(keyCode),'p')
%                             
%                         %Play sound when 'p' is pressed again, and then move on to
%                         %the next trial
%                         PTBPlaySoundFile('Sample.wav',{'end'},8);
%                             
%                             %resp = 'p';
...
sourceblock = {'Color Memory Test. Press any key.'};
...
for iTrial = 1:nTrials
    ...
    if  any(iTrial==1:11)
        ...
        if TestStims{iTrial,9,iBlock}==0
            ...
            PTBDisplayText('LEFT SURE     LEFT UNSURE',{[PTBScreenRes.width*.03 PTBScreenRes.height*.95]},{-1});
            PTBDisplayText('RIGHT UNSURE     RIGHT SURE',{[PTBScreenRes.width*.52 PTBScreenRes.height*.95]},{-1});
    ...
    elseif TestStims{iTrial,9,iBlock}==0
        ...
        PTBDisplayText('LEFT SURE     LEFT UNSURE',{[PTBScreenRes.width*.03 PTBScreenRes.height*.8]},{-1});
        PTBDisplayText('RIGHT UNSURE     RIGHT SURE',{[PTBScreenRes.width*.52 PTBScreenRes.height*.8]},{-1});
    ...
    if  TestStims{iTrial,8,iBlock}==2
        
        testDataFile{trial_counter,11}= sourceMem_cb(source_counter,1,iBlock);
    ...
    if iTrial==11
        orderblock = {'Order Memory Test. Press any key.'};

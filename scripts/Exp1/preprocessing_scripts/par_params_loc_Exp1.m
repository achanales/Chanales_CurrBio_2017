function par = par_params_loc(subject)
...
%set-up defaults (also makes sure spm is happily in the path)
spm_get_defaults; 
...
par.modelnum = 2; %AC Change
par.data_seg = 2; %AC Change 1 is experiment 2 is localizer

%Get directory stucture
par = par_params_directories_Exp1(subject,par);

%addpaths
addpath(genpath(par.pardir));
addpath(genpath(par.rawdir));
 
%% SPECIFY BASIC SCAN PARAMETERS
%----------Scan Params----------------

%Load Runs Info
excelFile = fullfile(par.scriptsdir,'run_info_files',[subject '_runinfo.txt']); %must be a text file
stimList = read_table(excelFile);
    %col 1 = study runs
    %col 2 = loclaizer runs
par.studyruns = stimList.col2(end);
...
for studyrun = 1:par.studyruns
    par.maxvol(studyrun) = stimList.col2(studyrun);
end
...
scpr = {[]; 'w';'sw';'u'; 'ur'; 'wur'; 'sur'; 'swur'}; % prepend scan with.. ([] means no prepend)
...
for ss = 1:length(scpr)
    pathparts = {[par.funcdir]; '/localizer'; ['/' scpr{ss} 'localizer'];'.nii,'};
    ...
    par.(['all' scpr{ss} 'scanfiles']) = [pathchunk{1} pathchunk{2} pathchunk{3} pathchunk{4} volnumchunk];
...
vpar.realflag.sep = 4;       % no Brice change
...
par.reslflag.mean = 1;  % no brice changes
...
% model vars...
par.timing.fmri_t0 = 8; 
par.timing.fmri_t = 16; 
...
% more model stuff
par.sess.scans = par.allsurscanfiles;

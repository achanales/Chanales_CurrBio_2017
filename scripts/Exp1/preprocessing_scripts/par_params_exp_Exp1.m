function par = par_params_exp(subject,par)
...
par.sliceorder = [1:2:par.numslice 2:2:par.numslice];  % interleaved ascending, but NOTE that Siemens starts with even slice # if total slices is even (e.g., 34) since we have an odd # of slices changes so that its it starts with 1 not 2
...
%% Reslice 
% reslice flags
par.reslflag.mask = 0; % AC Change after talking to Ed. 
par.reslflag.mean = 1;  
par.reslflag.interp = 4; 
par.reslflag.which = 0;   %0 if you dont want to resclice [2 1] to reslice all the images

    function [dataSheet_acc] = csv_psa_datamat2(analysisname,csvparams,tr)
        colstart = 1;
        ...
        rc = 2;
            for s = 1:length(csvparams.smoothingtypes)
                ...
                for i = 1:size(analysismats,1) 
                    ...
                    for j = 1:length(dataMat2)
                        %Summary csv
                        dataSheet_acc{rc,1} = csvparams.sub;
                        dataSheet_acc{rc,2} = maskname;
                        dataSheet_acc{rc,3} = dataMat2(2,j);
                        dataSheet_acc{rc,4} = dataMat2(3,j); %half 
                        dataSheet_acc{rc,5} = dataMat2(1,j); %overlapping pair

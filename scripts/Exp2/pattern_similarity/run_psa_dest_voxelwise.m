function[dataMat,oMat,noMat,sMat,combMat,srxo,srMat]= run_psa_dest_voxelwise(all_data,meta_runs_condensed,condensed_regs_all,S)
  ...
  %   RUN PSA
    ...
    for route = 1:4
        ...
        r_path2_o(:,:,route) = mean(all_data(:,:,r_inxh2_o),3); %tr x voxel pattern first half
        r_path2_e(:,:,route) = mean(all_data(:,:,r_inxh2_e),3); %tr x voxel pattern first half
    end

%     %%%%First half correlations%%%%%
...
noMat(1,:) = [nop1_1 nop2_1 nop1_2 nop2_2];
noMat(2,:) = repmat(1:num_vox,1,4); %vox number
noMat(3,:) = [vox_rank_nop1 vox_rank_nop2 vox_rank_nop1 vox_rank_nop2]; %ranking based on half 1 corrs
noMat(4,:) = [one1 one1 two2 two2]; %half
if mod(S.sub,2) == 1
    noMat(5,:) = [one1+2 two2+2 one1+2 two2+2]; %pair 
else
    noMat(5,:) = [one1 two2 one1 two2]; %pair 
end
...
[no_sr,~] = corr(vox_rank_nop1',vox_rank_nop2','type','Spearman');
...
no_percov = sum(no1_bin == no2_bin)/300;

srMat(1,:) = [mean(sp_sr) op_sr no_sr]; %spearman rank correlation
srMat(2,:) = [mean(sp_percov) op_percov no_percov]; %percent of voxels in same bin
srMat(3,:) = [1 2 3];

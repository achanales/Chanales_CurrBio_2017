function par_setcontrasts(subpar)
% parSetContrasts(subpar)  (study phase)
% Sets contrasts.
% 
...
conTable = read_table(['contrasts_M' num2str(par.modelnum) '.txt']);
...
% scan contrasts
for x = 1:length(conTable.col1)
    for y = 1:length(conTable.cols_names)-2;
        con_vals{x}(y) = eval(['conTable.col' num2str(y+2) '(' num2str(x) ')']);
    end
    con_name{x} = conTable.col1{x};
end
